<!--
  HourSeries.svelte — hand-rolled SVG vertical-bar series for contacts-by-hour.
  No chart library; sized to the max bucket, token colours. Mirrors the analytics
  TimeSeries but local to the event-stats route.
-->
<script lang="ts">
    ...
    >
      {#each buckets as b, i (b.label)}
        ...
        <rect
          ...
          fill="rgb(var(--brand))"
        >
          <title>{b.label}:00 — {b.count}</title>
        </rect>
      {/each}
    </svg>
    <div class="flex justify-between text-xs text-subtle">
      <span>{buckets[0]?.label}:00</span>
      <span>{buckets[buckets.length - 1]?.label}:00</span>
    </div>
  </div>
{/if}
